...
function PlanDialog({ plan, onClose, onChanged }) {
  ...
  return <dialog className="plan-dialog" ref={ref} onClose={onClose} onClick={(event) => { if (event.target === ref.current) ref.current.close(); }}><div className="plan-dialog__card"><button className="plan-dialog__close" type="button" onClick={() => ref.current?.close()} aria-label="Cerrar">×</button>
