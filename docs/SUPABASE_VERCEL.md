...
## Dos fotos de «Un detalle para ti»

Hay dos espacios fijos: **Tus regalos** y **Tus flores**. Antes de subir cada foto, el botón abre un formulario para elegirla y escribir el mismo `MEMORY_ADMIN_CODE` largo usado en el álbum; la clave de entrada de seis dígitos no sirve para subir. El archivo se guarda directamente en `our-memories/details/gifts/` o `our-memories/details/flowers/`. En cuanto existe una foto, se muestra en la tarjeta y desaparecen los controles de subida. No se necesita crear tablas, políticas ni variables nuevas.

**Plan alternativo desde Supabase:** en **Storage → our-memories**, crea las carpetas `details/gifts` y `details/flowers` si todavía no aparecen. Sube una imagen JPG, PNG o WebP dentro de la carpeta correspondiente. Al recargar la página, la tarjeta muestra el archivo de esa carpeta. Puedes subir la foto de los regalos ahora y comprobarla desde otro dispositivo; después de comprar las flores, sube la segunda desde el celular.
